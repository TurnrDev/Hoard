...
const ownOptions = computed(() =>
  ownCharacters.value
    ...
    .map((character) => ({ title: character.name, value: character.id })),
);
const destinationOptions = computed(() => [
  { title: "Campaign store", value: null },
  ...(campaign.value?.characters
    .filter((character) => character.is_active && !character.is_archived)
    .map((character) => ({ title: character.name, value: character.id })) ??
    []),
]);
const sourceCharacter = computed(() =>
  ownCharacters.value.find((character) => character.id === sourceId.value),
);
const inventoryCandidates = computed<PickerCandidate[]>(
  () =>
    sourceCharacter.value?.inventory.flatMap((entry) => {
  ...
  try {
    ...
    error.value =
      exception instanceof Error
        ? exception.message
        : "Unable to load character actions.";
  }
}
...
async function moveItem(): Promise<void> {
  if (!sourceId.value || !itemId.value || destinationId.value === undefined)
    return;
  try {
    await createInventoryTransaction(campaignId, {
      from_character_id: sourceId.value,
...
async function transferMoney(): Promise<void> {
  ...
  try {
    await createMoneyTransfer(campaignId, {
      from_character_id: sourceId.value,
      to_character_id: destinationId.value,
      amounts: { [transferDenomination.value]: transferAmount.value },
      description: description.value,
    });
...
async function exchangeMoney(): Promise<void> {
  ...
  try {
    await createMoneyExchange(campaignId, {
      character_id: exchangeCharacterId.value,
      given: { [givenDenomination.value]: givenAmount.value },
...
<template>
  <v-container style="max-width: 960px">
    <div class="d-flex align-center justify-space-between mb-6">
      <div>
        <div class="text-overline text-secondary">Character actions</div>
        <h1 class="text-h4">Move belongings and money</h1>
      </div>
    ...
    <v-alert type="info" variant="tonal" class="mb-4"
      >You can move items from your character, send your character’s money to
      another character or the campaign store, and exchange your own
      coins.</v-alert
    >
    <v-tabs v-model="tab"
      ><v-tab value="items">Move item</v-tab
      ><v-tab value="transfer">Send money</v-tab
      ><v-tab value="exchange">Exchange money</v-tab></v-tabs
    >
      ...
      <v-window-item value="items"
        ><v-card
          ><v-card-text>
            ...
            <v-btn
              color="primary"
              :disabled="!sourceId || !itemId || destinationId === undefined"
              @click="moveItem"
              >Move item</v-btn
            >
          </v-card-text></v-card
      ...
      <v-window-item value="transfer"
        ><v-card
          ><v-card-text>
            ...
            <v-btn
              color="primary"
              :disabled="!sourceId || destinationId === undefined"
              @click="transferMoney"
              >Send money</v-btn
            >
          </v-card-text></v-card
        ...
        ><v-card
          ><v-card-text>
            ...
            <v-row
              ><v-col
                ><v-select
                  v-model="givenDenomination"
                  :items="denominations"
                  label="Give denomination" /><v-text-field
                  v-model.number="givenAmount"
                  type="number"
                  min="1"
                  label="Give amount" /></v-col
              ><v-col
                ><v-select
                  ...
                  type="number"
                  min="1"
                  label="Receive amount" /></v-col
            ></v-row>
            <v-textarea v-model="description" label="Note (optional)" />
            <v-btn
              color="primary"
              :disabled="!exchangeCharacterId"
              @click="exchangeMoney"
              >Exchange money</v-btn
            >
          </v-card-text></v-card
        ></v-window-item
      >
