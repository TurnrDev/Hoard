...
onMounted(async () => {
  try {
    ...
    error.value = exception instanceof Error ? exception.message : 'Unable to initialise sign-in.'
  }
})
    ...
    <v-card class="pa-6" elevation="8">
      ...
      <v-card-text class="pt-6">
        ...
        <v-form @submit.prevent="submit">
          ...
          <v-btn block color="primary" type="submit" :loading="loading" :disabled="!csrfReady">Sign in</v-btn>
        </v-form>
      </v-card-text>
    </v-card>
  </v-container>
