...
const itemOptions = computed(() => items.value.map((item) => ({
  title: item.name,
  value: item.id,
  subtitle: [item.source_system ? `Source: ${item.source_system}` : 'Campaign custom', item.created_by_username ? `Created by: ${item.created_by_username}` : null].filter(Boolean).join(' · '),
})))
const actions = [
  ...
  <v-container fluid class="pa-md-8">
    ...
    <template v-if="campaign">
      <div class="d-flex flex-wrap align-center justify-space-between mb-6 ga-4">
        ...
        <div class="d-flex ga-2"><v-btn v-if="isGM" color="primary" prepend-icon="mdi-controller" :to="`/campaigns/${campaignId}/gm`">GM controls</v-btn><v-btn v-if="isGM" prepend-icon="mdi-plus" @click="openAction">Post action</v-btn><v-btn prepend-icon="mdi-package-variant-plus" @click="itemDialog = true">New item</v-btn></div>
      </div>
      ...
      <v-window v-model="tab" class="pt-4">
        ...
        <v-window-item value="items"><v-data-table :headers="[{title:'Item',key:'name'},{title:'Source',key:'source_system'},{title:'Creator',key:'created_by_username'}]" :items="items"><template #item.source_system="{ item }"><v-chip size="small">{{ item.source_system ?? 'Campaign custom' }}</v-chip></template><template #item.created_by_username="{ item }">{{ item.created_by_username ?? 'Upstream' }}</template></v-data-table></v-window-item>
