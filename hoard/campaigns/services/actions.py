...
def _normalise_coins(coins: CoinAmounts) -> dict[MoneyEntry.Denomination, int]:
    ...
    for denomination, amount in coins.items():
        try:
            ...
            raise ValidationError(f'Unknown currency denomination: {denomination}.') from error
...
def _ensure_character_has_coins(character: Character, coins: Mapping[MoneyEntry.Denomination, int]) -> None:
    ...
    if any(amount > available[denomination] for denomination, amount in coins.items()):
        raise ValidationError('A character cannot spend or exchange coins they do not hold.')
...
def transfer_item(
    *, source: Character, recipient: Character, item: InventoryItem, quantity: int, description: str = ''
) -> InventoryTransaction:
    """Transfer held inventory between two characters."""
    if quantity <= 0 or source.inventory.get(item, 0) < quantity:
        raise ValidationError('A character cannot transfer more of an item than they hold.')
    return post_inventory_transaction(
...
def grant_coins(*, recipient: Character, coins: CoinAmounts, description: str = '') -> MoneyTransaction:
    ...
    entries = [(system, denomination, -amount) for denomination, amount in amounts.items()]
    entries.extend((account, denomination, amount) for denomination, amount in amounts.items())
    return post_money_transaction(entries, description=description)


def spend_coins(*, spender: Character, coins: CoinAmounts, description: str = '') -> MoneyTransaction:
    ...
    entries = [(account, denomination, -amount) for denomination, amount in amounts.items()]
    entries.extend((system, denomination, amount) for denomination, amount in amounts.items())
    return post_money_transaction(entries, description=description)


def exchange_coins(
    *, character: Character, given: CoinAmounts, received: CoinAmounts, description: str = ''
) -> MoneyTransaction:
    ...
    given_value = sum(COPPER_VALUES[denomination] * amount for denomination, amount in given_amounts.items())
    received_value = sum(COPPER_VALUES[denomination] * amount for denomination, amount in received_amounts.items())
    ...
    entries = [(account, denomination, -amount) for denomination, amount in given_amounts.items()]
    entries.extend((account, denomination, amount) for denomination, amount in received_amounts.items())
    return post_money_transaction(entries, description=description)
...
def reverse_transaction(transaction_to_reverse: LedgerTransaction, *, description: str = '') -> LedgerTransaction:
    ...
    if isinstance(transaction_to_reverse, InventoryTransaction):
        return reverse_inventory_transaction(transaction_to_reverse, description=description)
    if isinstance(transaction_to_reverse, MoneyTransaction):
        return reverse_money_transaction(transaction_to_reverse, description=description)
    return reverse_experience_transaction(transaction_to_reverse, description=description)
