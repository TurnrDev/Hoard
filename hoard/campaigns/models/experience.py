...
class ExperienceAccount(models.Model):
    ...
    character = models.OneToOneField('campaigns.Character', null=True, blank=True, on_delete=models.CASCADE, related_name='experience_account')
    is_system = models.BooleanField(default=False)

    class Meta:
        constraints = [
            models.UniqueConstraint(fields=('campaign',), condition=Q(is_system=True), name='one_system_experience_account_per_campaign'),
...
class ExperienceEntry(ImmutableLedgerEntry):
    ...
    class Meta:
        constraints = [models.CheckConstraint(condition=~Q(amount=0), name='experience_entry_nonzero_amount')]
