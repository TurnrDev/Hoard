...
class MoneyAccount(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=('campaign',), condition=Q(is_system=True), name='one_system_money_account_per_campaign'),
...
class MoneyEntry(ImmutableLedgerEntry):
    ...
    transaction = models.ForeignKey(MoneyTransaction, on_delete=models.PROTECT, related_name='entries')
    account = models.ForeignKey(MoneyAccount, on_delete=models.PROTECT, related_name='entries')
    denomination = models.CharField(max_length=2, choices=Denomination.choices)

    class Meta:
        constraints = [models.CheckConstraint(condition=~Q(amount=0), name='money_entry_nonzero_amount')]
